...
- **Timeline** (bottom) — drag the two handles to trim the project start and end; click or drag the bar to scrub. **Zoom** continuously from 20% to 40x — drag the slider, use the +/- buttons, or hold Ctrl/Cmd and scroll over the timeline. You can also type an exact percentage in the box and press **Enter** (**Escape** abandons the edit) when you need finer positioning. Audio clips sit in their own lane below: drag the middle to move a clip in time, drag either **end** to trim it.
